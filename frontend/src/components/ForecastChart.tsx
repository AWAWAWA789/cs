 ...
 * 每个点生成一根模拟 K 线：以 price 为中心，添加随机波动模拟 OHLC。
 */
...
export function ForecastChart({
  ...
  const forecastScenarios = scenarios
    ...
    .map((s, idx) => {
      ...
      const simBars = waveToSimulatedBars(s.wave_sketch, lastClose);
  ...
  const option = {
    ...
    legend: {
      data: forecastScenarios.map((f) => `${f.scenario.name} (${(f.scenario.probability * 100).toFixed(0)}%)`),
      bottom: 0,
